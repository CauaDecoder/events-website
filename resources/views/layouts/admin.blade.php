        ...
        <div class="min-h-screen lg:grid lg:grid-cols-[17rem_1fr]">
            <aside class="hidden border-r border-white/10 bg-[#1c1917] text-white lg:flex lg:flex-col">
                ...
                <nav class="flex-1 space-y-1 p-4">
                    ...
                    <a href="{{ route('admin.subscriptions.index') }}" wire:navigate @class(['flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition', 'bg-[#e58775] font-medium text-[#1c1917]' => request()->routeIs('admin.subscriptions.*'), 'text-stone-300 hover:bg-white/5 hover:text-white' => ! request()->routeIs('admin.subscriptions.*')])><flux:icon.credit-card class="size-5" /> Assinaturas</a>
            ...
            <div class="min-w-0" data-page-transition>
                <header class="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-stone-200 bg-white/95 px-4 backdrop-blur dark:border-white/10 dark:bg-zinc-900/95 lg:hidden">
                    ...
                    <nav class="flex gap-1"><flux:button size="sm" variant="ghost" icon="squares-2x2" :href="route('admin.dashboard')" wire:navigate /><flux:button size="sm" variant="ghost" icon="users" :href="route('admin.users.index')" wire:navigate /><flux:button size="sm" variant="ghost" icon="credit-card" :href="route('admin.subscriptions.index')" wire:navigate /></nav>
                </header>
                {{ $slot }}
            </div>
        </div>
